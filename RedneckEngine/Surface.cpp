	...
	note("[Note] " + note)
{
	if (hr)
	{
		...
		note = "[Error String] " + WindowErrorClass::TranslateErrorCode(*hr) + "\n" + note;
	}
}
	...
	RedneckException(line, file)
{
	...
	if (hr)
	{
		note = "[Error String] " + WindowErrorClass::TranslateErrorCode(*hr) + note;
	}
}
...
const char* Surface::Exception::what() const noexcept
{
	...
	oss << RedneckException::what() << std::endl << GetNote();
	whatBuffer = oss.str();
