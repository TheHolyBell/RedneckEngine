...
public:
	...
	static void ShowDialogAsync(std::function<void(const std::string&)> _callback, const char* filter = "All Files(*.*)\0*.*");
	
	// @ Simple synchronous version of previous method, this one returns path if it has been chosen, otherwise - empty std::optional
	static std::optional<std::string> ShowDialog(const char* filter = "All Files(*.*)\0*.*");
};